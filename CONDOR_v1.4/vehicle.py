...
#      v['Performance']['MissionRange'] = m['Segment 2']['Distance'] # REMOVEME this needs to be the sum of all mission compoenets (Set after flyMission is executed)
  ...
  def scaleWeights(self): # AHS change how scale weights are done
      ...
      if self.Master['Code Selection']['rf_Run']:
        ...
        scaledEngineWeight = 1
        #w['NumEngines']*((0.1054*(MRP/w['NumEngines'])**2.+358*(MRP/w['NumEngines'])+2.757*10.**4.)/((MRP/w['NumEngines'])+1180))
      else:
        MRP = v['Powerplant']['MRP']
        scaledEngineWeight = w['NumEngines']*w['BaselineWeightPerEngine']
      
      try:
          scaledDriveSystemWeight = 1
          #(525.*(self.GW/1000.)**1.14)/((self.GW/MRP)**0.763*v['Main Rotor']['DiskLoading']**0.381)
      except:
